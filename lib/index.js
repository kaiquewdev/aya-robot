...
function robotTeletransportHandler(opts) {
  var self = this;

  self.position.move(opts);

  return self;
}
Robot.prototype.teletransport = robotTeletransportHandler;

function robotCommandHandler(input, opts) {
  var self = this;
  ...
  switch (input) {
    ...
    case 'M':
      ...
      break;
    case 'T':
      self.teletransport(opts);
      break;
  }

  return self;
}
Robot.prototype.command = robotCommandHandler;

function robotBatchHandler(input) {
  var self = this;

  input.split('').map(self.command.bind(self));

  return self;
}
Robot.prototype.batch = robotBatchHandler;
